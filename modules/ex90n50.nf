workflow EX90N50_SINGLE{
    main:
        SALMON_INDEX(params.assemblies)
        SALMON_QUASIMAPPING_SINGLE(params.reads, SALMON_INDEX.out)
...
workflow EX90N50_PAIRED{
    main:
        SALMON_INDEX(params.assemblies)
        SALMON_QUASIMAPPING_PAIRED(params.reads, SALMON_INDEX.out)
        ABUNDANCE_ESTIMATES_TO_MATRIX(SALMON_QUASIMAPPING.out)
        CONTIG_EXN50_STATISTIC(ABUNDANCE_ESTIMATES_TO_MATRIX.out)
    emit:
      CONTIG_EXN50_STATISTIC.out
...
process SALMON_QUASIMAPPING_SINGLE {
  label "Ex90N50"
...
process SALMON_QUASIMAPPING_PAIRED {
  label "Ex90N50"
  ...
  shell:
    """
    salmon quant -l A -i salmon_${name} -1 ${leftReads} -2 ${rightReads} -o salmon_quant_${name} -p !{params.threads}
    """
}
